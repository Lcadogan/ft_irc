            ...
            { 
                ...
                if (*sit == 'o')
                {
                    for( ; oit != args.end(); ++oit)
                        if (isalpha((*oit)[0]))
                        {
                            Client * user = server.find_client(*oit++);
                            if (user != nullptr)
                            {
                                if (channel->this_channel(user))
                                    buf_c == '+' ? channel->set_Oper(user) : channel->del_Oper(user);
                ...
                else if (*sit == 'l')
                {
                    if (buf_c == '-')
                        ...
                        for( ; oit != args.end(); ++oit)
                            if (isnumber((*oit)[0]))
                                {
                                    channel->set_Limit(std::stoul(*oit));
                                    break ;
                                }
                    ...
                    buf_c == '+' ? channel->flag_t |= true : channel->flag_t &= false;
                else if (*sit == 'i')
                    buf_c == '+' ? channel->flag_i |= true : channel->flag_i &= false;
                else if (*sit == 'n')
                    buf_c == '+' ? channel->flag_n |= true : channel->flag_n &= false;
