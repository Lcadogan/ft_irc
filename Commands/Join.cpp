...
void	                Join::execute()
{
    ...
    for (std::vector<std::string>::iterator ait = args1.begin(), sit = second_args.empty() ? second_args.end() : second_args.begin(); ait != args1.end(); ++ait)
    {
        ...
        if ((*ait)[0] != '#')
            continue ;
        Channel *channel = server.find_channel(*ait);
        if (channel == nullptr)
        {
            channel = new Channel(*ait, &client, password);
            server.getChannels().push_back(channel);
        }
        else if (!channel->is_Invit(&client) && (channel->this_channel(&client) || ( channel->limit > 0 && (channel->clients.size() + 1 > channel->limit)) || channel->flag_i || channel->password != password || (channel->flag_i && channel->password == password)))
            continue ;
        else
