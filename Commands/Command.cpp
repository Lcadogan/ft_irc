...
void							Command::Execute_prep()
{
    if (command == "PASS" || command == "QUIT")
        ;
    else if (command == "NICK" && !client.pass_flag)
        return ;
    else if (command == "USER" && client.getNick().empty())
        return ;
    else if (command != "NICK" && command != "USER" && !client.is_registred())
        return ; 
    execute();
}
...
void	Command::print_message(Client* other_client, Channel* channel)
{
    ...
    switch (res)
    {
        ...
        case (RPL_WELCOME):
            if (command.compare("USER"))
                ...
                message += "Welcome to the Russian Instagram... " + Client->getNick()
